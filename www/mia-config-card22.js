...
class MiaConfigCard extends HTMLElement {
    ...
    timeSelectorsToDecimal(hour, minute) {
        // Converte ore e minuti in formato HH.MM (non decimale!)
        // Es: 22 ore e 30 minuti = 22.30, 22 ore e 55 minuti = 22.55
        // La parte dopo il punto rappresenta i minuti (0-59), non una frazione
        return parseFloat(hour + '.' + String(minute).padStart(2, '0'));
    }
    ...
    renderConfigurationsByOverride(validConfigs, container) {
        ...
        for (const [name, configsList] of validConfigs) {
            for (const cfg of configsList) {
                ...
                if (cfg.type === 'schedule') {
                    ...
                    if (cfg.valid_from_ora !== undefined && cfg.valid_to_ora !== undefined) {
                        groupKey += `_${cfg.valid_from_ora}_${cfg.valid_to_ora}`;
                        groupTitle += ` | ⏰ ${this.formatTimeDisplay(cfg.valid_from_ora)}-${this.formatTimeDisplay(cfg.valid_to_ora)}`;
                    }
                    if (cfg.days_of_week) {
                        const days = cfg.days_of_week.sort().join(',');
                        groupKey += `_${days}`;
                    ...
                    // Standard - non raggruppa
                    groupKey = `standard_${name}_${cfg.id}`;
                    groupTitle = `⚙️ Standard: ${name}`;
                }
        ...
        const sortedGroups = Object.entries(groups).sort((a, b) => {
            // Prima per tipo (time > schedule > conditional > standard)
            const typeOrder = { time: 1, schedule: 2, conditional: 3, standard: 4 };
        ...
        for (const [groupKey, group] of sortedGroups) {
            ...
            if (group.type !== 'standard') {
                // Pulsante per eliminare l'intero gruppo
                html += `<button class="dc-btn-danger" onclick="window.dcDeleteOverrideGroup('${groupKey.replace(/'/g, "\\'")}', ${JSON.stringify(group.configs.map(c => ({ type: c.cfg.type, id: c.cfg.id }))).replace(/"/g, '&quot;')})">Elimina Gruppo</button>`;
            }
            html += `</div>`;
        ...
        // Aggiungi funzione per eliminare un intero gruppo
        window.dcDeleteOverrideGroup = async (groupKey, configsToDelete) => {
            if (!confirm(`Eliminare tutte le ${configsToDelete.length} configurazioni di questo gruppo di override?`)) {
                return;
            }
            
            try {
                for (const item of configsToDelete) {
                    await this.callMiaConfigService('delete_single_config', {
                        config_type: item.type,
                        config_id: parseInt(item.id)
                    });
                }
                this.showToast(`${configsToDelete.length} configurazioni eliminate!`);
                setTimeout(() => this.loadConfigurations(), 500);
            } catch (err) {
    ...
    renderWeeklyView(setupName, configs) {
        ...
        const conditionalConfigs = configs.filter(c => c.type === 'conditional');
        ...
        const daySegments = days.map(day => this.calculateDaySegments(day, standardConfig, timeConfigs, scheduleConfigs, conditionalConfigs));
    ...
    generateSegmentTooltip(segment) {
        ...
        if (segment.type === 'time') {
            ...
            tooltip += `Entità: ${segment.entity}<br>`;
